      ...
      {pillarsData.map((p, i) => (
        <div key={i} style={{
          ...
          <div style={{ background: p.color, padding: '20px 28px' }}>
            <div style={{
              fontFamily: DM, fontWeight: 600, fontSize: 11,
              letterSpacing: '0.14em', textTransform: 'uppercase',
              color: HM.tuscany, marginBottom: 6,
            }}>Pillar · {p.num}</div>
            <div style={{
              fontFamily: GRIFT, fontWeight: 800, fontSize: 42,
              color: HM.bone, letterSpacing: '0.01em', lineHeight: 1,
            }}>{p.name}</div>
          </div>
          {/* Card body */}
          <div style={{ padding: '20px 28px 24px', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <p style={{ fontFamily: DM, fontWeight: 400, fontSize: 18, lineHeight: 1.6, color: HM.ink, margin: 0 }}>{p.body}</p>
            <div style={{
              fontFamily: DM, fontWeight: 300, fontSize: 11,
              letterSpacing: '0.1em', textTransform: 'uppercase',
              color: HM.tuscany, marginTop: 16,
            }}>{p.tag}</div>
          </div>
        </div>
      ))}
