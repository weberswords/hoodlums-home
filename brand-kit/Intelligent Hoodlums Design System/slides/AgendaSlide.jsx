...
const AgendaSlide = ({
  ...
  items = [
    ...
    { time: '9:30',  duration: '15 min', title: 'Break',                note: 'Get coffee. Come back ready.' },
  ...
  <div style={{
    ...
    display: 'flex', flexDirection: 'column', fontFamily: DM,
  }}>
    <Masthead light />
    ...
    <div style={{ padding: '90px 80px 36px', flexShrink: 0 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          ...
          <h2 style={{
            fontFamily: GRIFT,
            fontWeight: 800, fontSize: 64, lineHeight: 1.0,
            color: HM.ink, margin: '0 0 20px', letterSpacing: '0.01em',
          }}>{workshopTitle}</h2>
          <RufousRule width={48} />
        </div>
        <div style={{
          fontFamily: DM, fontWeight: 300, fontSize: 13,
          letterSpacing: '0.1em', textTransform: 'uppercase',
          color: HM.cadet, textAlign: 'right',
        }}>{date}</div>
      </div>
    </div>
    ...
    <div style={{ padding: '0 80px 72px', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 0 }}>
      {items.map((item, i) => (
        <div key={i} style={{
          display: 'grid',
          gridTemplateColumns: '80px 68px 1fr auto',
          gap: '0 28px', alignItems: 'center',
          padding: '14px 0',
          borderBottom: `1px solid rgba(15,20,25,${i === items.length - 1 ? 0 : 0.1})`,
        }}>
          <div style={{
            fontFamily: DM, fontWeight: 300, fontSize: 14,
            color: HM.cadet, letterSpacing: '0.06em', fontVariantNumeric: 'tabular-nums',
          }}>{item.time}</div>
          <div style={{
            fontFamily: DM, fontWeight: 300, fontSize: 10, color: HM.tuscany,
            letterSpacing: '0.1em', textTransform: 'uppercase',
            border: `1px solid ${HM.tuscany}`, padding: '2px 6px',
            textAlign: 'center', whiteSpace: 'nowrap',
          }}>{item.duration}</div>
          <div style={{ fontFamily: GRIFT, fontWeight: 700, fontSize: 22, color: HM.ink, letterSpacing: '0.01em' }}>{item.title}</div>
          <div style={{
            fontFamily: DM, fontWeight: 300, fontSize: 13, color: HM.cadet,
            letterSpacing: '0.04em', maxWidth: 380, textAlign: 'right', lineHeight: 1.4,
          }}>{item.note}</div>
        </div>
      ))}
    </div>
